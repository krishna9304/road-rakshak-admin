    ...
    <HomeLayout header={"My Account"}>
            ...
            <button
              onClick={() => {
                if (user.isVerified) {
                  ...
                  axios
                    .post(`${BACKEND_URL}api/v1/admin/verifyuser`, {
                      email: user.email,
                    })
                    .then((res) => {
                      if (res.data.res) {
      ...
      <Modal
        ...
        onOk={() => {
          if (userData.newPass === userData.repass) {
            axios
              .post(`${BACKEND_URL}api/v1/admin/changeprofile`, userData)
              .then((res) => {
                if (res.data.res) {
                  notification.success({
                    message: "Success",
      ...
      <div className="w-full flex text-center justify-center items-center">
        <button
          ...
          className="py-2 px-4 bg-red-600 hover:bg-red-700 focus:ring-red-500 focus:ring-offset-red-200 text-white w-44 m-8 transition ease-in duration-200 text-center text-base font-semibold shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2  rounded-lg "
        >
          Logout
        </button>
      </div>
